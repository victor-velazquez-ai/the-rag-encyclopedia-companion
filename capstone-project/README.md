...
```bash
python -m rag_capstone.app.run                  # answers demo questions over the sample corpus
python -m rag_capstone.app.run "your question"  # ask your own
```
...
## How it maps back
Every stage links to the chapter that argues it and the `professional_rag_kit` module that implements it, so the
