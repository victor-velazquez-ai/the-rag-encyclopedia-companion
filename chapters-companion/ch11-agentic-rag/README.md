...
|---|---|---|---|
| 1 | `01_plan_act_reflect_loop.ipynb` · `01_plan_act_reflect_loop.py` | Plan-act-reflect loop, turn-capped, gated by the complexity classifier (§ "Agentic RAG") | [`professional_rag_kit/architectures/agentic/loop.py`](../../professional_rag_kit/architectures/agentic/) |
| 2 | `02_self_ask_ircot.ipynb` · `02_self_ask_ircot.py` | Multi-hop — Self-Ask (default) and IRCoT, interleave retrieval with reasoning (§ "Multi-hop retrieval") | [`professional_rag_kit/architectures/agentic/multi_hop.py`](../../professional_rag_kit/architectures/agentic/) |
| 3 | `03_mcp_tool_boundary.ipynb` | MCP — wrap each source as a server (Tools / Resources / Prompts), runtime discovery (§ "The MCP tool boundary") | [`professional_rag_kit/architectures/agentic/mcp.py`](../../professional_rag_kit/architectures/agentic/) |
| 4 | `04_entity_resolution_splink.ipynb` · `04_entity_resolution_splink.py` | Entity resolution — deterministic-then-Fellegi-Sunter, Splink, canonical IDs (§ "Entity resolution and the canonical layer") | [`professional_rag_kit/architectures/agentic/entity.py`](../../professional_rag_kit/architectures/agentic/) |
| 5 | `05_resolve_entity_tool.ipynb` | Normalize queries to canonical IDs *before* retrieval (the `resolve_entity` MCP tool) (§ "The ER pipeline and the resolve_entity tool") | [`professional_rag_kit/architectures/agentic/entity.py`](../../professional_rag_kit/architectures/agentic/) |

## The experiment — `reproduce.py`

```bash
make ch11            # launch the notebooks
python chapters-companion/ch11-agentic-rag/reproduce.py   # or just the headline experiment
```
...
```python
from professional_rag_kit.architectures.agentic import AgenticRAG, EntityResolver
...
`make setup && make up` (installs `professional_rag_kit`, loads sample data, starts Qdrant). The complexity
