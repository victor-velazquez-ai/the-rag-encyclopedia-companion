...
|---|---|---|---|
| 1 | `01_hnsw_tradeoff.ipynb` · `01_hnsw_tradeoff.py` | HNSW + the recall/latency/memory/dollar sweep (§ "The recall/latency/memory/dollar tradeoff") | [`professional_rag_kit/ingestion/indexing/ann.py`](../../professional_rag_kit/ingestion/indexing/) |
| 2 | `02_ann_families.ipynb` | IVF-PQ · ScaNN · DiskANN — choosing by constraint (§ "The recall/latency/memory/dollar tradeoff") | [`professional_rag_kit/ingestion/indexing/ann.py`](../../professional_rag_kit/ingestion/indexing/) |
| 3 | `03_filtered_ann.ipynb` · `03_filtered_ann.py` | Filterable HNSW vs. naive pre/post-filtering (§ "Filtered ANN: where naive systems silently break") | [`professional_rag_kit/ingestion/indexing/filtered.py`](../../professional_rag_kit/ingestion/indexing/) |
| 4 | `04_quantization.ipynb` · `04_quantization.py` | In-index int8 / binary / RaBitQ + rescore (§ "In-index quantization") | [`professional_rag_kit/ingestion/indexing/quantize.py`](../../professional_rag_kit/ingestion/indexing/) |
| 5 | `05_pgvector_vs_dedicated.ipynb` | pgvector(+scale) vs. a dedicated store (§ "pgvector versus a dedicated store") | [`professional_rag_kit/ingestion/indexing/store.py`](../../professional_rag_kit/ingestion/indexing/) |
| 6 | `06_freshness_compaction.ipynb` | Incremental insert, tombstones, compaction scheduling (§ "Incremental indexing and the freshness problem") | [`professional_rag_kit/ingestion/indexing/freshness.py`](../../professional_rag_kit/ingestion/indexing/) |

## The experiment — `reproduce.py`

```bash
make ch05            # launch the notebooks
python chapters-companion/ch05-vector-stores/reproduce.py   # or just the headline experiment
```
...
```python
from professional_rag_kit.ingestion import VectorStore
...
`make setup && make up` (installs `professional_rag_kit`, loads sample data, starts Qdrant). This store indexes
