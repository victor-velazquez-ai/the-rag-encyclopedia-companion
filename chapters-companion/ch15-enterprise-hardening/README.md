...
|---|---|---|---|
| 1 | `01_retrieval_acl_trimming.ipynb` · `01_retrieval_acl_trimming.py` | Stamp source ACLs, inject a server-side identity filter at query time (OWASP LLM08:2025) (§ "Access control at retrieval time") | [`professional_rag_kit/production/security/`](../../professional_rag_kit/production/security/) |
| 2 | `02_multi_tenancy.ipynb` | Physical namespace-per-tenant (fails closed) vs. shared-index + `tenant_id` filter (§ "Multi-tenancy isolation") | [`professional_rag_kit/production/security/`](../../professional_rag_kit/production/security/) |
| 3 | `03_two_sided_pii.ipynb` · `03_two_sided_pii.py` | Redact before indexing AND re-redact retrieved chunks; embeddings as PII (Vec2Text) (§ "PII, redaction, and data residency") | [`professional_rag_kit/production/security/`](../../professional_rag_kit/production/security/) |
| 4 | `04_injection_defense_in_depth.ipynb` · `04_injection_defense_in_depth.py` | Spotlighting, hardened model/detector, tool filtering, output controls (OWASP LLM01:2025) (§ "Indirect prompt injection") | [`professional_rag_kit/production/security/`](../../professional_rag_kit/production/security/) |

## The experiment — `reproduce.py`

```bash
make ch15            # launch the notebooks
python chapters-companion/ch15-enterprise-hardening/reproduce.py   # or just the headline experiment
```
...
```python
from professional_rag_kit.production.security import SecureRetriever
...
`make setup && make up` (installs `professional_rag_kit`, loads sample data, starts Qdrant). The metadata filters
