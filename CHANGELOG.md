...
## Naming convention alignment (2026-06-29)
- Adopted the shared companion-repo naming used across the book series:
  - `chapters/` → **`chapters-companion/`** (one folder per chapter; the front door).
  - `capstone/` → **`capstone-project/`**; the runnable app now lives in
    `capstone-project/rag_capstone/app/` (importable, hyphen-safe).
  - `ragkit/` → **`professional_rag_kit/`** (distribution name **`professional-rag-kit`**) — all
    189 import references updated; **121 tests pass**.
- Added `[tool.pytest.ini_options] pythonpath` so tests resolve both packages without an install.
- README/docs repointed to the new names; status updated from "scaffold" to **live**; author set to
...
- Root `README.md` with the master-map table (chapter → folder → run → `professional_rag_kit` module → reproduces).
- Infra: `pyproject.toml` (professional_rag_kit + extras), `Makefile` (`make chNN` / `make reproduce`),
  `docker-compose.yml` (Qdrant), `.env.example`, `.gitignore`.
- Docs: `SETUP`, `HOW-TO-USE` (the two journeys + folder anatomy), `PROVIDER-SWAP`.
- `professional_rag_kit/` package skeleton: `core` (chunk schema, config/providers) + Part modules; exemplar
  `retrieval/rerank` stubs.
- Exemplar chapter folder `ch07-reranking/` (the per-chapter README template).
- Remaining in Phase 1: the other 14 chapter READMEs + full `professional_rag_kit` submodule stubs + `capstone-project/`
  and `data/` specs.
...
- **`professional_rag_kit.core` implemented + tested**: `Chunk` (pydantic, with `with_context` for Contextual
  Retrieval), `Config`/`load_config` (YAML + env-override provider selection), `ProviderRegistry`
  (the one-line swap mechanism).
- **`professional_rag_kit.production.generation` implemented**: `GroundedGenerator` (Ch 13) — context-faithful
...
- **Ingestion + retrieval stack implemented**:
  ...
  - Package `__init__`s export the conveniences (`from professional_rag_kit.retrieval import HybridRetriever`, ...).
...
- **First per-chapter walkthroughs (notebooks) — all offline, no API key:**
  ...
  - Ch 3 chunking, Ch 6 hybrid+RRF, Ch 8 lost-in-the-middle + MMR, Ch 14 retrieval metrics — each a
    paired `.py` (verified by execution) + `.ipynb`, narrating one technique against `professional_rag_kit`.
- **Eval harness + runnable reproduction (Ch 14), all offline:**
  - `professional_rag_kit.eval`: `GoldenSet` (jsonl loader), `Harness` (scores a retriever's nDCG@k/Recall@k/MRR over
  ...
  - `professional_rag_kit.eval.suite` (`make reproduce`): body-only BM25 vs BM25 body+title RRF fusion on the golden
    set — a real measured delta (+0.033 nDCG@5; fusion recovers a title-only-vocabulary query) with a
    cost column, no API key. `chapters-companion/ch14-evaluation/reproduce.py` runs it.
  - **42 unit tests pass.**
- **Capstone — the assembled end-to-end pipeline, runnable (`make capstone`):**
  - `capstone-project/app/pipeline.py` — `RAGPipeline` wires the verdict stack: hybrid retrieve (BM25 + optional
  ...
  - `capstone-project/rag_capstone/app/run.py` (`python -m rag_capstone.app.run "question"`) — answers over the sample corpus;
    abstains on off-corpus questions. Verified end-to-end offline.
  - 4 capstone tests; **46 unit tests pass.**
- **Full `professional_rag_kit` library implemented (all 16 chapters' components).** Remaining modules added
...
- Full `professional_rag_kit` library (core · ingestion · retrieval · architectures · production · eval), BYO-API.
