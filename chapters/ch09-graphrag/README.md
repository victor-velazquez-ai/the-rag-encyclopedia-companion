...
|---|---|---|---|
| 1 | `01_when_a_graph_earns_its_cost.ipynb` | The test for a graph-shaped problem — global vs. multi-hop vs. local (§ "Why similarity search cannot answer a global question") | [`ragkit/architectures/graph/`](../../ragkit/architectures/graph/) |
| 2 | `02_lightrag.ipynb` · `02_lightrag.py` | LightRAG — dual-level retrieval + incremental updates, the default (§ "LightRAG") | [`ragkit/architectures/graph/light_rag.py`](../../ragkit/architectures/graph/) |
| 3 | `03_lazy_graphrag.ipynb` · `03_lazy_graphrag.py` | LazyGraphRAG — co-occurrence graph, ~0.1% index cost (§ "LazyGraphRAG") | [`ragkit/architectures/graph/lazy_graph.py`](../../ragkit/architectures/graph/) |
| 4 | `04_microsoft_graphrag.ipynb` | Full GraphRAG — Leiden communities, reports, Global/Local/DRIFT modes (§ "Microsoft GraphRAG") | [`ragkit/architectures/graph/graph_rag.py`](../../ragkit/architectures/graph/) |
| 5 | `05_hipporag_ppr.ipynb` | HippoRAG 2 — Personalized PageRank over an LLM graph (§ "HippoRAG 2", frontier) | [`ragkit/architectures/graph/`](../../ragkit/architectures/graph/) |

## The experiment — `reproduce.py`

```bash
make ch09            # launch the notebooks
python chapters/ch09-graphrag/reproduce.py   # or just the headline experiment
```
...
```python
from ragkit.architectures.graph import GraphRAG
...
`make setup && make up` (installs `ragkit`, loads sample data, starts Qdrant). The flat baseline a
