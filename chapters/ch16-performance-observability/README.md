...
|---|---|---|---|
| 1 | `01_semantic_cache.ipynb` · `01_semantic_cache.py` | Semantic cache + the grey-zone problem; vCache learned per-prompt threshold (§ "Semantic caching") | [`ragkit/production/serving/`](../../ragkit/production/serving/) |
| 2 | `02_latency_budget.ipynb` | The staged budget — generation dominates, the reranker is the retrieval-half surprise (§ "The staged-pipeline latency budget") | [`ragkit/production/serving/`](../../ragkit/production/serving/) |
| 3 | `03_tail_latency_p99.ipynb` | P99 over the mean; TTFT, inter-token latency, streaming hides total latency (§ "Tail latency") | [`ragkit/production/serving/`](../../ragkit/production/serving/) |
| 4 | `04_continuous_batching.ipynb` | vLLM continuous batching + PagedAttention + chunked prefill (§ "Batching and streaming") | [`ragkit/production/serving/`](../../ragkit/production/serving/) |
| 5 | `05_prompt_cache_and_routing.ipynb` · `05_prompt_cache_and_routing.py` | Stable-prefix-first prompt caching + FrugalGPT/RouteLLM model routing (§ "Cutting cost") | [`ragkit/production/serving/`](../../ragkit/production/serving/) |
| 6 | `06_tracing.ipynb` · `06_tracing.py` | OTel GenAI + OpenInference span kinds (RETRIEVER→RERANKER→LLM) (§ "Observability: tracing standards") | [`ragkit/production/observability/`](../../ragkit/production/observability/) |
| 7 | `07_drift_detection.ipynb` | Three-surface drift; centroid-cosine / MMD / classifier, never univariate KS on embeddings (§ "Drift detection") | [`ragkit/production/observability/`](../../ragkit/production/observability/) |
| 8 | `08_feedback_loop.ipynb` | Thumbs → annotation queue → eval dataset; online LLM-judge; inline guardrail (§ "Feedback loops") | [`ragkit/production/observability/`](../../ragkit/production/observability/) |

## The experiment — `reproduce.py`

```bash
make ch16            # launch the notebooks
python chapters/ch16-performance-observability/reproduce.py   # or just the headline experiment
```
...
```python
from ragkit.production.serving import SemanticCache, Router
from ragkit.production.observability import Tracer
...
`make setup && make up` (installs `ragkit`, loads sample data, starts Qdrant). The reranker that is the
