...
|---|---|---|---|
| 1 | `01_recursive_baseline.ipynb` · `01_recursive_baseline.py` | Structure-aware recursive split, ~200 tok + overlap — the baseline (§ "The baseline") | [`ragkit/ingestion/chunking/recursive.py`](../../ragkit/ingestion/chunking/) |
| 2 | `02_semantic_chunking.ipynb` | Embedding-breakpoint semantic chunking (§ "Semantic chunking") | [`ragkit/ingestion/chunking/semantic.py`](../../ragkit/ingestion/chunking/) |
| 3 | `03_propositional.ipynb` | Propositional / Dense-X atomic chunking (§ "Propositional (Dense-X) chunking") | [`ragkit/ingestion/chunking/propositional.py`](../../ragkit/ingestion/chunking/) |
| 4 | `04_late_chunking.ipynb` | Late Chunking — embed whole doc, then segment + pool (§ "Late Chunking") | [`ragkit/ingestion/chunking/late.py`](../../ragkit/ingestion/chunking/) |
| 5 | `05_contextual_retrieval.ipynb` · `05_contextual_retrieval.py` | Anthropic Contextual Retrieval — LLM context + BM25 (§ "Anthropic Contextual Retrieval") | [`ragkit/ingestion/chunking/contextual.py`](../../ragkit/ingestion/chunking/) |
| 6 | `06_small_to_big.ipynb` | Small-to-big / sentence-window / auto-merging (§ "Hierarchical retrieval") | [`ragkit/ingestion/chunking/hierarchical.py`](../../ragkit/ingestion/chunking/) |
| 7 | `07_metadata.ipynb` | Chunk-time metadata enrichment (§ "Metadata enrichment at chunk time") | [`ragkit/ingestion/chunking/metadata.py`](../../ragkit/ingestion/chunking/) |

## The experiment — `reproduce.py`

```bash
make ch03            # launch the notebooks
python chapters/ch03-chunking/reproduce.py   # or just the headline experiment
```
...
```python
from ragkit.ingestion import Chunker
...
`make setup && make up` (installs `ragkit`, loads sample data, starts Qdrant). Chunking consumes the
